---
title: "Posterior Predictive Check Analysis: igt_mod - nnlboth_tic - batch_001"
output: 
  html_document:
    toc: true
    toc_float: true
...
ppc_summary_file <- "/Users/icd/Library/CloudStorage/Dropbox/Projects/modeling_mIGT/Data/ppc/stats/ppc_summary_task-igt_mod_model-nnlboth_tic_group-batch_001.csv"     # ppc_summary_{task}_{model}_{group}.csv
...
This document presents posterior predictive check (PPC) analysis for the **nnlboth_tic** model (**`r model_type`** type) applied to the **igt_mod** task with group type **batch_001**.
...
if(nrow(wsls_data) > 0) {
  ...
  wsls_agg <- wsls_data %>%
    ...
    summarize(
      ...
      pred_mean = quantile(unlist(simulated_mean), 0.5, na.rm = TRUE),
      pred_lower = quantile(unlist(simulated_mean), 0.025, na.rm = TRUE),
      pred_upper = quantile(unlist(simulated_mean), 0.975, na.rm = TRUE),
...
This analysis examines how well the nnlboth_tic model can reproduce key behavioral patterns observed in the igt_mod task. The model's fit quality is evaluated through posterior predictive checks, comparing observed statistics to those generated from model simulations.
...
```{r}
# Group-level statistics approach
group_stats <- ppc_summary %>%
  group_by(statistic) %>%
  summarize(
    # Observed - a single value across all subjects
    observed = mean(observed, na.rm = TRUE),
    
    # Calculate quantiles from these aggregate statistics
    p_2.5 = quantile(unlist(list(simulated_mean)), 0.025, na.rm = TRUE),
    p_97.5 = quantile(unlist(list(simulated_mean)), 0.975, na.rm = TRUE)
  )

kable(group_stats, caption = "Summary of Diffs") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
```{r}
subject_diffs <- ppc_summary %>%
  group_by(subject_id, statistic) %>%
  summarize(
    diff = observed - simulated_mean,
    .groups = "drop"
  )

# Then get summary of differences across subjects
diff_summary <- subject_diffs %>%
  group_by(statistic) %>%
  summarize(
    mean_diff = mean(diff, na.rm = TRUE),
    se_diff = sd(diff, na.rm = TRUE) / sqrt(sum(!is.na(diff))),
    t_value = mean_diff / se_diff,
    p_value = 2 * pt(-abs(t_value), sum(!is.na(diff)) - 1)
  )

kable(diff_summary, caption = "Summary of Diffs") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
```{r}
# Create scatterplot with 1:1 line
ggplot(ppc_summary %>% filter(statistic == "play_ratio"), 
       aes(x = observed, y = simulated_mean)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") +
  labs(title = "Shrinkage plot for play ratio",
