# IGT NNL Delta Model
igtNNLDELTAModel <- R6::R6Class("igtNNLDELTAModel",
  inherit = ModelBase,
  ...
  public = list(
    ...
    simulate_choices = function(trials, parameters) {
      ...
      for (t in 1:n_trials) {
        ...
        # Calculate utility using tanh transformation (as in the Stan model)
        utility <- tanh(wins[t]) * gain - tanh(losses[t]) * loss
    ...
    calculate_loglik = function(data, parameters) {
      ...
      for (t in 1:nrow(data)) {
        ...
        # Calculate utility using tanh transformation
        utility <- tanh(win) * gain - tanh(lose) * loss
        
        # Update chosen deck value with delta rule
