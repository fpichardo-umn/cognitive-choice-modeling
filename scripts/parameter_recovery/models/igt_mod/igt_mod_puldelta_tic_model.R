igt_modPULDELTATICModel <- R6::R6Class("igt_modPULDELTATICModel",
                           inherit = ModelBase,
                           ...
                           public = list(
                             ...
                             get_parameter_info = function() {
                               return(list(
                                 con = list(range = c(0, 5)),  # Changed range for TIC
                             ...
                             simulate_choices = function(trials, parameters) {
                               ...
                               sensitivity <- as.numeric(3^parameters$con - 1)
                               ...
                               for(t in 1:n_trials) {
                                 ...
                                 if (!is.na(forced_choices[t])) {
                                   ...
                                   # Calculate decision probability with constant sensitivity
                                 ...
                                 if(choices[t] == 1) {
                                   ...
                                   utility <- if(outcome > 0) {
                                      outcome**(as.numeric(parameters$mag)) * as.numeric(parameters$gain) 
                                   } else {
                                     (abs(outcome)**(as.numeric(parameters$mag))) * as.numeric(parameters$loss) * -1
                                   }
                             ...
                             calculate_loglik = function(trials, choices, outcomes, parameters) {
                               ...
                               for(t in 1:n_trials) {
                                 ...
                                 # Calculate choice probability with constant sensitivity
                                 info <- sensitivity * self$ev[shown_deck]
                                 prob_play <- 1 / (1 + exp(-info))
                                 ...
                                 if(choices[t] == 1) {
                                   ...
                                   # Calculate utility using PUL formula
                                   utility <- log(1 + abs(outcome)) * 
                                     ifelse(outcome > 0, parameters$gain, parameters$loss) * 
                                     sign(outcome)
