...
igt_modPVLDECAYDDMB1BModel <- R6::R6Class("igt_modPVLdecayDDMB1BModel",
  inherit = ModelBase,
    ...
    simulate_choices = function(trials, parameters) {
      ...
      for(t in 1:n_trials) {
        ...
        if(choices[t] == 1) {
          ...
          # Update expected value with decay rule
    ...
    calculate_loglik = function(trials, choices, RTs, outcomes, parameters) {
      ...
      for(t in 1:n_trials) {
        ...
        if(choices[t] == 1) {
          ...
          # Update expected value with decay rule (add utility to decayed value)
          self$ev[shown_deck] <- self$ev[shown_deck] + utility
        }
      }
