igt_modDUALPBBUModel <- R6::R6Class("igt_modDUALPBBUModel",
  inherit = ModelBase,
  ...
  public = list(
    ...
    simulate_choices = function(trials, parameters) {
      ...
      log_odds <- vector("numeric", n_trials)
      ...
      # Initialize Bayesian Belief system probabilities (3 systems in bbu model)
      model_probs <- c(0.33, 0.33, 0.34)  # Slight bias to random at start
      ...
      for (t in 1:n_trials) {
        ...
        # Decision policy using consistency parameter
        log_odds[t] <- as.numeric(3^parameters$consistency - 1) * deck_value
        ...
        if (!is.na(forced_choices[t])) {
          ...
          prob_play <- 1 / (1 + exp(-log_odds[t]))
          
          # Make choice
          choices[t] <- rbinom(1, 1, prob_play)
        }
        
        # 4. UPDATE MODELS (only if chosen)
        if (choices[t] == 1) {
          # Generate outcome
          ...
          likelihoods <- c(likelihood_mb_ev, likelihood_recency, likelihood_random)
          ...
          # 4c. UPDATE MODEL-BASED EV COMPONENT (BAYESIAN) - direct updates
          
          # Update Beta distributions for win/loss probabilities
          if (current_outcome > 0) {
            alpha_win[curDeck] <- alpha_win[curDeck] + 1 # Increment win successes
            beta_loss[curDeck] <- beta_loss[curDeck] + 1 # Increment loss failures
            ...
            beta_win[curDeck] <- beta_win[curDeck] + 1  # Increment win failures
            alpha_loss[curDeck] <- alpha_loss[curDeck] + 1  # Increment loss successes
          ...
          # 4d. UPDATE RECENCY MODEL - direct update
          
          # Update recency information directly (no weighting)
    ...
    calculate_loglik = function(trials, choices, outcomes, parameters) {
      ...
      sigma_loss <- rep(1.0, 4)
      recent_outcomes <- rep(0.0, 4)
      model_probs <- c(0.33, 0.33, 0.34)  # Three belief systems in bbu model
      trial_counts <- c(1, 1, 1, 1)
      precomputed_decay <- 1.0 / (1:50)^parameters$decay_factor
      ...
      for (t in 1:n_trials) {
        ...
        # Calculate log-odds with consistency parameter
        log_odds <- as.numeric(3^parameters$consistency - 1) * deck_value
        
        # Calculate probability of observed choice
        prob_play <- 1 / (1 + exp(-log_odds))
        ...
        if (choices[t] == 1) {
          ...
          if (current_outcome > 0) {
            alpha_win[curDeck] <- alpha_win[curDeck] + 1
            beta_loss[curDeck] <- beta_loss[curDeck] + 1
            ...
            beta_win[curDeck] <- beta_win[curDeck] + 1
            alpha_loss[curDeck] <- alpha_loss[curDeck] + 1
