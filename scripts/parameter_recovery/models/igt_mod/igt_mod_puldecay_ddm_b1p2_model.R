...
igt_modPULDECAYDDMB1P2Model <- R6::R6Class("igt_modPULDECAYDDMB1P2Model",
  inherit = ModelBase,
      ...
      for(t in 1:n_trials) {
        ...
        if(choices[t] == 1) {
          ...
          utility <- if(outcome > 0) {
            parameters$gain * outcome^parameters$mag
          } else {
            -parameters$loss * abs(outcome)^parameters$mag
          }
          
          # Add utility to the chosen deck (pure decay rule)
