...
igt_modDUALPBBUPDDMB1P2Model <- R6::R6Class("igt_modDUALPBBUPDDMB1P2Model",
  inherit = ModelBase,
  ...
  public = list(
    ...
    simulate_choices = function(trials, parameters) {
      ...
      sensitivity <- (1:n_trials/10)^parameters$drift_con
      ...
      for (t in 1:n_trials) {
        ...
        drift_rate <- deck_value * sensitivity[t]
        
        # Get block-specific boundary and tau
        if (t <= 20) {  # First block
          boundary_val <- parameters$boundary1
          tau_val <- parameters$tau1
        } else {  # Later blocks
          boundary_val <- parameters$boundary
          tau_val <- parameters$tau
        }
        ...
        if (!is.na(forced_choices[t])) {
          ...
          if (choices[t] == 1) {  # Play decision
            ddm_result <- rdiffusion(1, 
                                  a = boundary_val,  # Boundary separation
                                  t0 = tau_val,  # Non-decision time
                                  z = parameters$beta * boundary_val,  # Starting point
            ...
            ddm_result <- rdiffusion(1, 
                                  ...
                                  t0 = tau_val,  # Non-decision time
                                  z = parameters$beta * boundary_val,  # Starting point
                                  v = -drift_rate)  # Negative drift rate for pass
          }
          RTs[t] <- ddm_result$rt
        } else {
          ...
          choices[t] <- ifelse(ddm_result$response == "upper", 1, 0)
        ...
        tryCatch({
          if(choices[t] == 1) {
            ...
            trial_loglik[t] <- log(ddiffusion(
              ...
              a = boundary_val,
              t0 = tau_val,
              z = parameters$beta * boundary_val,
              v = drift_rate
            ))
          } else {
          ...
          # If ddiffusion fails, assign very low log-likelihood
          trial_loglik[t] <<- -1000
        })
