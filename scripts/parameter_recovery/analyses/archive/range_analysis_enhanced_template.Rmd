---
title: "Enhanced Parameter Range Analysis with Behavioral Integration: {{TASK}} - {{MODEL}} - {{GROUP}}"
output: 
  html_document:
    toc: true
    toc_float: true
...
# File paths to recovery and PPC data
recovery_file <- "{{RECOVERY_FILE}}"
ppc_subject_file <- "{{PPC_SUBJECT_FILE}}"
ppc_model_file <- "{{PPC_MODEL_FILE}}"
ppc_block_file <- "{{PPC_BLOCK_FILE}}"

# Load recovery data
recovery_data <- read.csv(recovery_file)

# Load PPC data if files exist
has_ppc_data <- file.exists(ppc_subject_file) && file.exists(ppc_model_file)
...
if (has_ppc_data) {
  ...
  has_block_data <- file.exists(ppc_block_file) && ppc_block_file != ""
  if (has_block_data) {
...
analyze_recovery_by_range <- function(data, parameter_name, n_bins = 5) {
  ...
  breaks <- seq(min(param_data$true_value, na.rm = TRUE),
                max(param_data$true_value, na.rm = TRUE),
                length.out = n_bins + 1)
  
  param_data$bin <- cut(param_data$true_value, breaks = breaks)
...
find_optimal_ranges <- function(data, min_correlation = 0.5, max_rmse_percentile = 0.25) {
...
This document analyzes how parameter recovery quality varies across different parameter ranges for the **{{MODEL}}** model ({{MODEL_TYPE}} type) applied to the **{{TASK}}** task using the group type **{{GROUP}}**.

This enhanced analysis integrates both parameter recovery metrics and behavioral performance to identify optimal parameter ranges with:
  ...
  for(param in parameters) {
    ...
    for(stat in unique(behavior_by_param$statistic)) {
      ...
      if(nrow(stat_data) >= 3) {
        ...
        # Add table with correlation between parameter and observed behavior
        if(nrow(stat_data) >= 3) {
          # Calculate correlation
          cor_val <- cor(stat_data$bin_center, stat_data$mean_observed, 
                        use = "pairwise.complete.obs")
          
          # Create a small data frame for the correlation
          cor_df <- data.frame(
            Parameter = param,
            Metric = stat,
            Correlation = cor_val
          ...
          # Display as table
          kable(cor_df, caption = paste("Correlation between parameter and metric"),
                digits = 3) %>% 
...
## Parameter-Behavior Correlation Analysis

This section analyzes correlations between model parameters and behavioral metrics to identify which parameters are most important for realistic behavior.

```{r param_behavior_cor, fig.width=10, fig.height=8}
# Only run if PPC data is available
if(has_ppc_data) {
  # Create correlation matrix
  corr_matrix <- create_parameter_behavior_correlation(recovery_data, ppc_subject_data)
  ...
  if(!is.null(corr_matrix) && nrow(corr_matrix) > 0 && ncol(corr_matrix) > 0) {
    ...
    p <- create_behavior_parameter_heatmap(recovery_data, ppc_subject_data, top_n = 15)
    if(!is.null(p)) {
      print(p)
    }
    
    # Identify behaviorally significant parameters
    ...
    if(!is.null(sig_params) && nrow(sig_params) > 0) {
      # Display table
      kable(sig_params, caption = "Parameters Ranked by Behavioral Significance",
            digits = 3) %>% 
        kable_styling(bootstrap_options = c("striped", "hover")) %>%
        print()
    }
  ...
  if(!is.null(behavior_sig) && !is.null(optimal_ranges) && 
    ...
    if(length(optimal_params) > 0) {
      for(param in optimal_params) {
        ...
        opt_range_df <- data.frame(
          Parameter = param,
          Min_Value = opt_range$optimal_min,
          Max_Value = opt_range$optimal_max
        )
        
        kable(opt_range_df, caption = paste("Optimal Range for", param),
              digits = 3) %>% 
          kable_styling(bootstrap_options = c("striped", "hover")) %>%
          print()
