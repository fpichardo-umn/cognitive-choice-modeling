...
setup_directories <- function(task, create_missing = TRUE) {
  ...
  # Parameter recovery directories
  SIM_DIR <- file.path(DATA_DIR, "sim")
  PARAMS_DIR <- file.path(SIM_DIR, "params")
  TXT_SIM_DIR <- file.path(SIM_DIR, "txt")
  RDS_SIM_DIR <- file.path(SIM_DIR, "rds")
  FIT_SIM_DIR <- file.path(SIM_DIR, "fit")
  REC_SIM_DIR <- file.path(SIM_DIR, "recovery")
...
initialize_model <- function(model_name, task_name, task, PR_DIR) {
  model <- NULL
...
# Initialize task
initialize_task <- function(task_name, PR_DIR) {
  task <- NULL
  source_path <- file.path(PR_DIR, "tasks", "base_task.R")
  ...
  source_path <- file.path(PR_DIR, "tasks", task_name, paste0(task_name, "_task.R"))
