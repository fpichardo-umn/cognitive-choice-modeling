...
print_usage() {
  echo "Usage: $0 -m <model_names> -f <fit_config> -d <data_config> -e <email> -t <model_type> -k <task> -g <group_type> -s <source> [-S <session>] [-n]"
  echo "Example: $0 -m \"ev_ddm,pvl_ddm\" -f default -d full -e your@email.edu -t fit -k igt_mod -g group_hier -s ahrb -S 00"
  echo "Options:"
...
DRY_RUN=false
while getopts ":m:f:d:e:t:k:g:s:S:n" opt; do
  case $opt in
...
if [ -z "$MODEL_NAMES" ] || [ -z "$USER_EMAIL" ] || [ -z "$TASK" ] || [ -z "$GROUP_TYPE" ] || [ -z "$SOURCE" ]; then
  echo "Error: Model names, email address, task, group type, and source are required."
  print_usage
fi
...
generate_r_call() {
  ...
  # Base command
  local cmd="Rscript $R_SCRIPT -m $model -t $MODEL_TYPE -k $TASK -g $GROUP_TYPE -s $SOURCE"
  ...
  cmd="$cmd --n_subs \${n_subs} --n_trials \${n_trials} --RTbound_min_ms \${RTbound_min_ms} --RTbound_max_ms \${RTbound_max_ms} --rt_method \${rt_method} --n_warmup \${n_warmup} --n_iter \${n_iter} --n_chains \${n_chains} --adapt_delta \${adapt_delta} --max_treedepth \${max_treedepth} $dry_run_flag"
...
for MODEL_NAME in "${MODEL_ARRAY[@]}"; do
  ...
  if $DRY_RUN; then
    ...
    job_id=$(sbatch --parsable \
      --job-name=$JOB_NAME \
      --export=ALL,JOB_NAME=$JOB_NAME,MODEL_NAME=$MODEL_NAME,FIT_CONFIG=$FIT_CONFIG,DATA_CONFIG=$DATA_CONFIG,USER_EMAIL=$USER_EMAIL,MODEL_TYPE=$MODEL_TYPE,TASK=$TASK,GROUP_TYPE=$GROUP_TYPE,SOURCE=$SOURCE,SESSION=$SESSION \
      $SBATCH_SCRIPT)
    if [ $? -eq 0 ]; then
