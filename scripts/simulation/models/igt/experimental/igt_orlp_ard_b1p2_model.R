...
# Individual ORL-ARD Model (Perseverance Separated)
...
igtORLARDB1P2PersSepModel <- R6::R6Class("igtORLARDB1P2PersSepModel",
                                         inherit = ModelBase,
                                         ...
                                         public = list(
                                           ...
                                           get_parameter_info = function() {
                                             # Parameters are identical to the original model
                                             return(list(
                                           ...
                                           simulate_choices = function(trials, parameters, task_params) {
                                             ...
                                             for (t in 1:n_trials) {
                                               ...
                                               # --- MODIFICATION START ---
                                               # 1. Calculate value-based utility (EV + EF) for all decks
                                               value_utility <- ev + ef * parameters$betaF
                                               ...
                                               for (i in 1:4) {
                                                 for (j in 1:3) {
                                                   ...
                                                   advantage_drift <- parameters$urgency + 
                                                     ...
                                                     wswd_minus * value_utility[other_deck_idx]
                                           ...
                                           calculate_loglik = function(data, parameters, task_params) {
                                             ...
                                             for (t in 1:n_trials) {
                                               ...
                                               if (rt >= RTbound_min && rt <= RTbound_max && rt != 999) {
                                                 ...
                                                 if (rt_adj <= 1e-5) {
                                                   trial_loglik[t] <- -Inf
                                                   # (Learning update below will still run, which is correct)
                                                 } else {
