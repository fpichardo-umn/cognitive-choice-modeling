...
# Individual VPPdelta-ARD Model (12 Accumulators, 3 per deck, "All-Win-First")
# Value-Perseverance-Prospect model with delta-rule learning
# For a deck to be chosen, ALL 3 of its accumulators must finish before 
# ALL 9 accumulators from the other 3 decks.

igtVPPDELTAPARDB1P2Model <- R6::R6Class("igtVPPDELTAPARDB1P2Model",
                                       inherit = ModelBase,
                                       ...
                                       public = list(
                                           ...
                                           for (t in 1:n_trials) {
                                             ...
                                             for (i in 1:4) {
                                               for (j in 1:3) {
                                                 ...
                                                 ev_component_i <- parameters$w * ev[i]
                                                 ev_component_other <- parameters$w * ev[other_deck_idx]
                                                 
                                                 # 2. Apply advantage scaling ONLY to the EV component
                                                 advantage_drift <- parameters$urgency + 
                                                   wswd_plus * ev_component_i + 
                                                   wswd_minus * ev_component_other
                                                 
                                                 # 3. Add the Perseverance component (weighted by 1-w) as a simple bias
                                                 drift_rates[k] <- advantage_drift + (1 - parameters$w) * pers[i]
                                             ...
                                             # 4. Update expected value using delta rule (VPPdelta rule)
                                         ...
                                         calculate_loglik = function(data, parameters, task_params) {
                                           ...
                                           for (t in 1:n_trials) {
                                             ...
                                             # 4. Update expected value (VPPdelta rule)
