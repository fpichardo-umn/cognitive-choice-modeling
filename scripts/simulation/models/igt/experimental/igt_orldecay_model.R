    ...
    simulate_choices = function(trials, parameters, task_params) {
      ...
      for (t in 1:n_trials) {
        ...
        if (wins[t] >= losses[t]) {
          ...
          # Decay chosen deck BEFORE applying update
          self$ef[choices[t]] <- self$ef[choices[t]] * (1 - Drew)
          self$ev[choices[t]] <- self$ev[choices[t]] * (1 - Drew)
          
          # Set chosen deck values directly to prediction errors (no learning rate)
          self$ef[choices[t]] <- PEfreq
          self$ev[choices[t]] <- PEval
        } else {
          # Net loss
