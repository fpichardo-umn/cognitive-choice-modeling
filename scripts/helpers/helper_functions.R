...
get_data_dir <- function(task) {
  file.path(here::here(), "Data", task)
}
...
extract_params <- function(param_names, n_subs = 1, num_to_view = 10, drop_lp = TRUE, 
  ...
  if (n_subs == 1) {
    ...
    # For hierarchical models, use all subjects if we have fewer than num_to_view
    # Otherwise, sample without replacement
    if (n_subs <= num_to_view) {
      sampled_indices_subs <- 1:n_subs  # Use all subjects
    } else {
      sampled_indices_subs <- sort(sample(1:n_subs, num_to_view, replace = FALSE))
    }
