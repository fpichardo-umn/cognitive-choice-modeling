...
```{r subject-ess}
cat(sprintf("- **Minimum ESS ratio (worst)**: %.3f\n", subj_agg$ess$min))
cat(sprintf("- **Median ESS ratio**: %.3f\n", subj_agg$ess$median))
cat(sprintf("- **Subjects with ESS ratio < %.2f**: %d\n\n", 
            thresholds$thresholds$ess_ratio$problematic, subj_agg$ess$n_problematic))
```
...
if (!is.null(diagnostic_results$subject_analysis$worst_subjects)) {
  ...
  worst_df <- data.frame(
    Subject = sapply(worst, function(x) x$subject_id),
    Status = sapply(worst, function(x) x$status),
    `Worst R-hat` = sprintf("%.4f", sapply(worst, function(x) x$worst_rhat)),
    `Min ESS Ratio` = sprintf("%.3f", sapply(worst, function(x) x$min_ess_ratio %||% NA)),
    `Problem Score` = sprintf("%.1f", sapply(worst, function(x) x$problem_score)),
    check.names = FALSE,
    stringsAsFactors = FALSE
  )
  
  kable(worst_df, row.names = FALSE)
}
```
...
if (!is.null(diagnostic_results$subject_analysis$subject_summaries)) {
  ...
  all_df <- data.frame(
    ...
    `Min ESS Ratio` = sprintf("%.3f", sapply(summaries, function(x) x$min_ess_ratio %||% NA)),
