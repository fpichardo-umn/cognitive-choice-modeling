...
subtitle: "{{MODEL}} - {{TASK}} - Subject {{SUBJECT_ID}}"
...
- Raw samples are correlated (each draw depends on previous draws)
- ESS adjusts for this correlation
- Higher ESS = more information about the posterior

**Two types:**
- **Bulk ESS**: For estimating means and medians
- **Tail ESS**: For estimating quantiles and credible intervals

## Interpretation Guide

```{r ess-interpretation}
ess_interp <- data.frame(
...
**Rule of thumb:** Aim for ESS > 400 for stable estimates.
...
cat(sprintf("- **Minimum ESS**: %.0f (%.1f%% of %.0f total samples)\n", 
            ess_info$min, ess_info$min_ratio * 100, diagnostic_results$metadata$total_samples))
cat(sprintf("- **Median ESS**: %.0f (%.1f%%)\n", 
            ess_info$median, ess_info$median_ratio * 100))
cat(sprintf("- **Status**: %s\n\n", ess_info$status))

if (any(ess_info$ratios > 1.0, na.rm = TRUE)) {
  cat("ℹ️ **Note**: Some ESS values exceed 100%. This is valid and indicates negative autocorrelation (anticorrelation) in the chains, which actually improves efficiency beyond independent samples.\n\n")
}
...
create_ess_histogram(ess_info$ratios, thresholds)
```
...
param_ess <- data.frame(
  Parameter = names(ess_info$values),
  `ESS Bulk` = round(ess_info$values, 0),
  `ESS Ratio` = sprintf("%.1f%%", ess_info$ratios * 100),
  check.names = FALSE,
  stringsAsFactors = FALSE
)

param_ess <- param_ess[order(ess_info$values), ]
...
if (ess_info$n_problematic > 0) {
  ...
  prob_idx <- which(ess_info$ratios < thresholds$thresholds$ess_ratio$problematic)
...
cat(sprintf("- **Divergence count**: %.0f\n", div_info$count))
cat(sprintf("- **Total transitions**: %.0f\n", div_info$total_transitions))
...
if (!is.null(div_info$by_chain)) {
  ...
  for (i in seq_along(div_info$by_chain)) {
    cat(sprintf("  - Chain %d: %.0f divergences\n", i, div_info$by_chain[i]))
  }
  cat("\n")
}
```
...
if (!is.null(diagnostic_results$sampling$treedepth)) {
  ...
  cat(sprintf("- **Maximum allowed**: %.0f\n", td_info$max_allowed))
  cat(sprintf("- **Hits**: %.0f (%.2f%% of transitions)\n", td_info$hits, td_info$rate * 100))
  cat(sprintf("- **Status**: %s\n\n", td_info$status))
...
```{r density-by-chain, fig.width=12, fig.height=8, results='asis'}
if (!is.null(fit)) {
