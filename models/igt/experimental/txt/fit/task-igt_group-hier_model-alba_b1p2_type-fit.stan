...
transformed parameters {
  ...
  array[N] real<lower=0.001, upper=3> A;     // LBA starting point variability
  array[N] real<lower=0.001, upper=3> sv;    // LBA drift variability
  ...
  A = to_array_1d(inv_logit(mu_pr[8] + sigma[8] .* to_vector(A_pr)) * 2.999 + 0.001);
  sv = to_array_1d(inv_logit(mu_pr[9] + sigma[9] .* to_vector(sv_pr)) * 2.999 + 0.001);

  V1 = to_array_1d((inv_logit(mu_pr[10] + sigma[10] .* to_vector(V1_pr)) - 0.5) * 20);
  V2 = to_array_1d((inv_logit(mu_pr[11] + sigma[11] .* to_vector(V2_pr)) - 0.5) * 20);
  V3 = to_array_1d((inv_logit(mu_pr[12] + sigma[12] .* to_vector(V3_pr)) - 0.5) * 20);
  V4 = to_array_1d((inv_logit(mu_pr[13] + sigma[13] .* to_vector(V4_pr)) - 0.5) * 20);
}
model {
  mu_pr ~ normal(0, 1);
...
generated quantities {
  ...
  real mu_wd = inv_logit(mu_pr[6]) * 19.999 + 0.001;
  real mu_ws = inv_logit(mu_pr[7]) * 19.999 + 0.001;
  real mu_A = inv_logit(mu_pr[8]) * 2.999 + 0.001;
  real mu_sv = inv_logit(mu_pr[9]) * 2.999 + 0.001;
  real mu_V1 = (inv_logit(mu_pr[10]) - 0.5) * 20;
  real mu_V2 = (inv_logit(mu_pr[11]) - 0.5) * 20;
  real mu_V3 = (inv_logit(mu_pr[12]) - 0.5) * 20;
  real mu_V4 = (inv_logit(mu_pr[13]) - 0.5) * 20;
}
