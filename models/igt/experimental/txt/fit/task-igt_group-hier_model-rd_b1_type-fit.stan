...
functions {
  ...
  real igt_rd_model(array[] int choice, array[] real RT, int T, vector V_subj,
                    vector boundaries, real tau, real urgency, real drift_con) {
    real log_lik = 0.0;
  ...
  real partial_sum(array[] int slice_n, int start, int end,
    ...
    for (n in start:end) {
      ...
      log_lik += igt_rd_model(
          ...
          V_subj, boundary_subj[n][1:Tsubj[n]], tau[n], urgency[n], drift_con[n]
      );
    }
    return log_lik;
  }
...
transformed parameters {
  ...
  array[N] real<lower=0, upper=20> urgency;
  ...
  drift_con   = to_array_1d(inv_logit(mu_pr[5] + sigma[5] .* to_vector(drift_con_pr)) * 19.999 + 0.001);
...
generated quantities {
  ...
  real mu_drift_con = inv_logit(mu_pr[5]) * 19.999 + 0.001;
