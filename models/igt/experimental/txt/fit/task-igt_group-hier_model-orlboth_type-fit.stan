// Optimized Hierarchical ORLBoth Model for the Iowa Gambling Task
functions {
  ...
  real partial_sum_func(array[] int slice_n, int start, int end,
                        array[,] int choice, array[,] real wins, array[,] real losses, 
                        array[] int Tsubj,
                        array[] real Arew, array[] real Apun, array[] real decay,
                        array[] real K, array[] real betaF, array[] real betaP) {
    real log_lik = 0.0;
...
transformed parameters {
  ...
  decay = to_array_1d(inv_logit(mu_pr[1] + sigma[1] .* to_vector(decay_pr)));
  K     = to_array_1d(inv_logit(mu_pr[2] + sigma[2] .* to_vector(K_pr)) * 5);
  betaF = to_array_1d(mu_pr[3] + sigma[3] .* to_vector(betaF_pr));
  betaP = to_array_1d(mu_pr[4] + sigma[4] .* to_vector(betaP_pr));
