...
functions {
  ...
  // VSE-RDM trial-level function with learning
  real igt_vse_rdm_model(
    ...
    for (t in 1:T) {
      ...
      // VSE update rule
      // 1. Update exploitation values: decay then add utility to chosen deck
      local_ev_exploit = local_ev_exploit * (1 - decay);
      local_ev_exploit[choice[t]] += curUtil;
      
      // 2. Update exploration values: reset chosen, update unchosen
      local_ev_explore[choice[t]] = 0.0;
      for (d in 1:4) {
        if (d != choice[t]) {
...
parameters {
  ...
  real explore_alpha_pr; // Exploration learning rate
  real explore_bonus_pr; // Exploration bonus
}
...
model {
  ...
  vector[4] ev_explore_init = rep_vector(0.0, 4);
  
  target += igt_vse_rdm_model(
