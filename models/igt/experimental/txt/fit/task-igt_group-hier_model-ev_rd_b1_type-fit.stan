...
transformed parameters {
  ...
  wgt_pun   = to_array_1d(inv_logit(mu_pr[3] + sigma[3] .* to_vector(wgt_pun_pr)));
  wgt_rew   = to_array_1d(inv_logit(mu_pr[4] + sigma[4] .* to_vector(wgt_rew_pr)));
...
generated quantities {
  ...
  real mu_wgt_pun   = inv_logit(mu_pr[3]);
  real mu_wgt_rew   = inv_logit(mu_pr[4]);
