...
transformed parameters {
  ...
  boundary1 = to_array_1d(inv_logit(to_vector(boundary1_pr)) * 4.99 + 0.001);
  boundary  = to_array_1d(inv_logit(to_vector(boundary_pr)) * 4.99 + 0.001);
  tau1 = to_array_1d(inv_logit(to_vector(tau1_pr)) .* (to_vector(minRT) - RTbound - 0.02) * 0.95 + RTbound);
  tau  = to_array_1d(inv_logit(to_vector(tau_pr)) .* (to_vector(minRT) - RTbound - 0.02) * 0.95 + RTbound);
  
  urgency = to_array_1d(inv_logit(to_vector(urgency_pr)) * 19.999 + 0.001);
  wd = to_array_1d(inv_logit(to_vector(wd_pr)) * 9.999 + 0.001);
  ws = to_array_1d(inv_logit(to_vector(ws_pr)) * 9.999 + 0.001);
  
  drift_con = to_array_1d(inv_logit(to_vector(drift_con_pr)) * 3);
  wgt_pun   = to_array_1d(inv_logitto_vector(wgt_pun_pr)));
  wgt_rew   = to_array_1d(inv_logit(to_vector(wgt_rew_pr)));
  update    = to_array_1d(inv_logit(to_vector(update_pr)));
}
