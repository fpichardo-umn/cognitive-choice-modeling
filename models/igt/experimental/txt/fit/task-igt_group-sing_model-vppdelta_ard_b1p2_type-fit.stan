functions {
  ...
  // Combined VPP-Delta-ARD model function with reduced redundant calculations.
  real igt_vppdelta_ard_model_lp(
        array[] int choice, array[] real wins, array[] real losses, array[] real RT,
        vector ev_init, vector pers_init, int T, 
        real gain, real loss, real update, real epP, real epN, real K, real w,
...
model {
  ...
  // Combined VPP-Delta-ARD model computation
  target += igt_vppdelta_ard_model_lp(choice, wins, losses, RT, ev, pers, T, 
                                      gain, loss, update, epP, epN, K, w,
