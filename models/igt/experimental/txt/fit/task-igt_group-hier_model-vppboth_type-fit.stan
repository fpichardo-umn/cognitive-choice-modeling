// Hierarchical VPPBoth Model for the Iowa Gambling Task
functions {
...
transformed parameters {
  ...
  epP    = to_array_1d(-5 + inv_logit(mu_pr[5] + sigma[5] .* to_vector(epP_pr)) * 10);
  epN    = to_array_1d(-5 + inv_logit(mu_pr[6] + sigma[6] .* to_vector(epN_pr)) * 10);
...
generated quantities {
  ...
  real<lower=-5, upper=5> mu_epP;
  real<lower=-5, upper=5> mu_epN;
  ...
  mu_epP    = -5 + inv_logit(mu_pr[5]) * 10;
  mu_epN    = -5 + inv_logit(mu_pr[6]) * 10;
