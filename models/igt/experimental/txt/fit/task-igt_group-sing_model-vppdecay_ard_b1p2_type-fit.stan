functions {
  ...
  // Combined VPP-Decay-ARD model function with reduced redundant calculations.
  real igt_vppdecay_ard_model_lp(
        array[] int choice, array[] real wins, array[] real losses, array[] real RT,
        vector ev_init, vector pers_init, int T, 
        real gain, real loss, real decay, real epP, real epN, real K, real w,
    ...
    for (t in 1:T) {
      ...
      real diff12 = V[1] - V[2]; real sum12 = V[1] + V[2];
      real diff13 = V[1] - V[3]; real sum13 = V[1] + V[3];
      real diff14 = V[1] - V[4]; real sum14 = V[1] + V[4];
      real diff23 = V[2] - V[3]; real sum23 = V[2] + V[3];
      real diff24 = V[2] - V[4]; real sum24 = V[2] + V[4];
      real diff34 = V[3] - V[4]; real sum34 = V[3] + V[4];
      ...
      // Add utility to chosen deck (no delta rule)
...
model {
  ...
  // Combined VPP-Decay-ARD model computation
  target += igt_vppdecay_ard_model_lp(choice, wins, losses, RT, ev, pers, T, 
                                      gain, loss, decay, epP, epN, K, w,
