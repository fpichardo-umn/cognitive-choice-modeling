// Individual PVLdelta-ARD Model for the Iowa Gambling Task
...
functions {
  ...
  // PVLdelta-ARD trial-level function
  real igt_ard_model(
  ...
  real partial_sum(array[] int slice_n, int start, int end,
    ...
    for (n in start:end) {
      ...
      log_lik += igt_ard_model(
          ...
          ev, Tsubj[n],
          sensitivity, update[n], loss[n], gain[n],
...
transformed parameters {
  ...
  gain      = to_array_1d(inv_logit(gain_pr) * 2);
