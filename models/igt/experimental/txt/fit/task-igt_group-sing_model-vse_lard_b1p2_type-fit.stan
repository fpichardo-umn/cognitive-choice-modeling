// Single-Subject VSE-ARD Model for the Iowa Gambling Task
...
functions {
  ...
  real igt_ard_model(
    ...
    for (t in 1:T) {
      ...
      // --- MODIFICATION START ---
      for (i in 1:4) {
        // 1. Get the exploit components
        // (vector[3] of the same value for the current deck)
        vector[3] exploit_i = rep_vector(local_ev_exploit[i], 3);
        // (vector[3] of the values for the 3 other decks)
        vector[3] exploit_other = local_ev_exploit[other_indices[i]];
        
        // 2. Apply advantage scaling ONLY to the exploit components
        vector[3] advantage_drift = rep_vector(urgency, 3) +
                                    wd * (exploit_i - exploit_other);
                                    
        // 3. Add the explore component as a simple bias
        //    (applies the same explore value to all 3 accumulators for deck i)
        drift_rates[k:k+2] = advantage_drift + local_ev_explore[i];
...
parameters {
  ...
  real gain_pr;
  real decay_pr;
  real explore_alpha_pr;
  real explore_bonus_pr;
}
