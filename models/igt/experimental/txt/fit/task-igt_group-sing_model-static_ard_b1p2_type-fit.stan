...
functions {
  ...
  real igt_ard_model(
      ...
      vector boundaries, vector taus, real urgency, real wd, real ws, // ARD params
    ...
    for (i in 1:4) {
      for (j in 1:3) {
        ...
        drift_rates[k] = urgency + 
                         wswd_plus * deck_values[i] + 
                         wswd_minus * deck_values[other_deck_idx];
        k += 1;
      }
    }
...
transformed parameters {
  ...
  real<lower=0.001, upper=10> ws; // ARD parameter
