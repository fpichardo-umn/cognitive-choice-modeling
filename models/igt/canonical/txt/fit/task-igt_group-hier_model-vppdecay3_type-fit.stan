// Optimized Hierarchical VPP-Decay Model for the Iowa Gambling Task
functions {
...
transformed parameters {
  ...
  epP   = to_array_1d(mu_pr[3] + sigma[3] .* to_vector(epP_pr));
  epN   = to_array_1d(mu_pr[4] + sigma[4] .* to_vector(epN_pr));
  K     = to_array_1d(inv_logit(mu_pr[5] + sigma[5] .* to_vector(K_pr)));
  w     = to_array_1d(inv_logit(mu_pr[6] + sigma[6] .* to_vector(w_pr)));
}
...
generated quantities {
  ...
  mu_epP   = mu_pr[3];
  mu_epN   = mu_pr[4];
  mu_K     = inv_logit(mu_pr[5]);
  mu_w     = inv_logit(mu_pr[6]);
}
