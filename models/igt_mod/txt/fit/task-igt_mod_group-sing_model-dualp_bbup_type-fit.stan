functions {
  ...
  vector igt_dual_process_model_lp(
      ...
      array[] real pattern_prediction, array[] real pattern_strength,
      // Initial values 
    ...
    // Pre-compute decay weights for efficiency - KEEPING EXACTLY AS ORIGINAL
    ...
    for (t in 1:Tsub) {
      ...
      if (choice[t] == 1) {
        ...
        //--- 4c. UPDATE MODEL-BASED EV COMPONENT (BAYESIAN) WITH update_weight_mb ---
        ...
        //--- 4d. UPDATE RECENCY MODEL with update_weight_mb ---
      ...
      //--- 5. APPLY DECAY TO ALL MODELS - KEEPING EXACTLY AS ORIGINAL ---
...
model {
  ...
  decay_factor_pr ~ normal(0, 1);  // KEEPING ORIGINAL
  ...
  vector[4] final_value = igt_dual_process_model_lp(
    ...
    pattern_prediction, pattern_strength,
    initial_mf_ev,
    T, consistency,
    update_rate_mf,
