functions {
  // Fitting NNL Decay DDM model with 1st block and rest boundary
  vector igt_model_lp(
    ...
    for (t in 1: Tsub) {
      ...
      // Compute utility using NNL formula (tanh)
      curUtil = tanh(outcome[t] * (outcome[t] > 0 ? gain : loss));

      // Update expected values (decay rule)
      local_ev *= (1 - decay);  // Decay all values
      if (choice[t] == 1) {
        local_ev[curDeck] += curUtil;  // Add utility to chosen deck
      }
...
parameters {
  ...
  real<lower=-3, upper=3> decay_pr;      // Decay parameter
}
