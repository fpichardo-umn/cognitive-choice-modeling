functions {
  vector igt_model_lp(
    ...
    for (t in 1:Tsub) {
      ...
      // Compute utility
      curUtil = tanh(abs(outcome[t])) * (outcome[t] > 0 ? gain : -loss);
...
transformed parameters {
  ...
  real<lower=0, upper=100> gain;
  real<lower=0, upper=100> loss;
  real<lower=0, upper=1>  update;
  
  con    = inv_logit(con_pr) * 10 - 5;
  gain   = inv_logit(gain_pr) * 100;
  loss   = inv_logit(loss_pr) * 100;
