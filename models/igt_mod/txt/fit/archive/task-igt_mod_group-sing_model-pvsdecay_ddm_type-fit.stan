functions {
  vector igt_model_lp(
    ...
    for (t in 1:Tsub) {
      ...
      // Calculate drift rate for DDM - this drives the RT-choice process
      ...
      // Update expected values with decay only
      local_ev -= decay * local_ev; // Decay every deck
      local_ev[curDeck] += curUtil * choice[t]; // Update selected deck without additional learning rate
