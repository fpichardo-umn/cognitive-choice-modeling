...
generated quantities {
  int subid = sid;
  real log_lik = 0;  // Sum of trial log likelihoods
  ...
  {  // Local block for efficiency
    ...
    for (t in 1:T) {
      int curDeck = shown[t];
      real info = sensitivity[t] * ev[curDeck];
      
      // Store log likelihood for this trial
      log_lik += bernoulli_logit_lpmf(choice[t] | info);
      
      // Update EVs (if choice was made)
      real curUtil = pow(abs(outcome[t]), gain) * (outcome[t] > 0 ? 1 : -1 * loss);
      
      ev -= decay * ev; // Decay every deck
      ev[curDeck] += curUtil * update * choice[t]; // Update selected deck unless choice 0, update 0
    }
  }
}
