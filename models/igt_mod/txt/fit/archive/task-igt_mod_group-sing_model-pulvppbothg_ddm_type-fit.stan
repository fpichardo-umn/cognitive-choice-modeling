functions {
  // Fitting PUL-VPP Both DDM model with general perseveration
  vector igt_model_lp(
			...
			real update, real decay, real k, real w, real ep
			) {
    // Define values
    ...
    for (t in 1: Tsub) {
      ...
      drift_rates[t] = w * (sensitivity[t] * local_ev[curDeck]) + (1-w) * gen_pers;
      ...
      // Update expected values (combined rule)
      local_ev -= decay * local_ev; // Decay every deck
      local_ev[curDeck] += curUtil * update * choice[t]; // Update selected deck
