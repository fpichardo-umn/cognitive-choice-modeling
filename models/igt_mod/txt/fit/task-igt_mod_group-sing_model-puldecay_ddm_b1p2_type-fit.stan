functions {
  // Fitting PUL Decay DDM model with 1st block and rest boundary
  vector igt_model_lp(
    ...
    for (t in 1: Tsub) {
      ...
      // Compute utility
      if (outcome[t] >= 0) {
        curUtil = gain * pow(outcome[t], mag);
      } else {
        curUtil = -loss * pow(abs(outcome[t]), mag);
      }

      // Update expected values - decay rule
      // Apply the general decay
...
parameters {
  ...
  real<lower=-3, upper=3> mag_pr;        // Outcome magnitude sensitivity
  real<lower=-3, upper=3> decay_pr;      // Retention/decay rate
}
