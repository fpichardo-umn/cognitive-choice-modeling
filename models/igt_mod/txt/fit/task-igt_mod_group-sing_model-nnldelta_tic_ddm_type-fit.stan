functions {
  ...
  vector igt_model_lp(
    ...
    for (t in 1: Tsub) {
      ...
      drift_rates[t] = local_ev[curDeck] * sensitivity; // Drift scaling

      // Compute utility
      curUtil = tanh(outcome[t]) * (outcome[t] > 0 ? gain : -loss);

      // Update expected values (both rule)
      local_ev[curDeck] += (curUtil - local_ev[curDeck]) * update * choice[t]; // Update selected deck unless choice 0, update 0
