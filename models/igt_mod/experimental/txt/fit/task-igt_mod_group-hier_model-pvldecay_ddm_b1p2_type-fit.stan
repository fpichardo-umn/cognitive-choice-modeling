// Optimized Hierarchical PVL-Decay-DDM b1p2 Model for IGT_MOD with Parallelization
functions {
  real igt_model(
      array[] int choice, array[] int shown, array[] real outcome,
      array[] real RT, vector ev, int Tsub,
      vector boundaries, vector taus, real beta,
    ...
    for (t in 1:Tsub) {
      ...
      if (choice[t] == 1) {
        local_ev[curDeck] += curUtil;
      }

      // Build indices for vectorized likelihood
      if (RT[t] != 999) {
        if (choice[t] == 1) {
          play_count += 1;
  ...
  real partial_sum(array[] int slice_n, int start, int end,
                   ...
                   array[] real gain, array[] real loss, array[] real decay) {
    real log_lik = 0.0;
    ...
    for (n in start:end) {
      ...
      log_lik += igt_model(
          ...
          beta[n], sensitivity, gain[n], loss[n], decay[n]
      );
    }
    return log_lik;
  }
...
parameters {
  ...
  // Removed hard bounds on _pr parameters
...
model {
  ...
  target += reduce_sum(partial_sum,
                       ...
                       Tsubj, choice, shown, outcome, RT,
                       boundary_subj, tau_subj,
                       beta, drift_con, gain, loss, decay);
}
