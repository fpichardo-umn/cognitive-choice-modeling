---
title: "Parameter Recovery Analysis: igt_mod - ev - batch_001"
output: 
  html_document:
    toc: true
    toc_float: true
...
recovery_file <- "/Users/icd/Library/CloudStorage/Dropbox/Projects/modeling_mIGT/Data/sim/recovery/recovery_igt_mod_ev_batch_001.csv"
...
This document presents parameter recovery analysis for the **ev** model (RL type) applied to the **igt_mod** task with group type **batch_001**.

## Recovery Metrics

### Raw Metrics

```{r raw_metrics, echo=FALSE}
kable(stats$overall[1:4], caption = "Raw Recovery Metrics") %>% 
...
kable(stats$overall[5:7], caption = "Standardized Recovery Metrics") %>% 
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
kable(stats$by_parameter[, c("parameter", "parameter_type", "std_correlation", "std_rmse", "std_bias")],
...
neg_nlin_params <- recovery_data %>%
  filter(grepl("neg_nlin_params", parameter, ignore.case = TRUE))
...
if(nrow(decay_params) > 0) {
  ...
  decay_params[decay_params$parameter == "retend",]$true_value = 1 - decay_params[decay_params$parameter == "retend",]$true_value
  decay_params[decay_params$parameter == "retend",]$recovered_value = 1 - decay_params[decay_params$parameter == "retend",]$recovered_value
  decay_params[decay_params$parameter == "retend",]$parameter = "decay"
...
temp_params <- recovery_data %>%
  filter(grepl("temp|beta|tau|con|drift_con", parameter, ignore.case = TRUE))
...
# Plot heatmap
corrplot::corrplot(cor_matrix, type = "upper", tl.col = "black", tl.srt = 45, addCoef.col = "black")
```

## Conclusion

This analysis examines parameter recovery performance for the ev model. Key metrics include correlation between true and recovered parameters, RMSE (root mean square error), bias, and relative bias.
