---
title: "Posterior Predictive Check Analysis: igt_mod - pvldelta_tic_ddm - batch_001"
output: 
  html_document:
    toc: true
    toc_float: true
...
ppc_summary_file <- "/Users/icd/Library/CloudStorage/Dropbox/Projects/modeling_mIGT/Data/ppc/stats/ppc_summary_task-igt_mod_model-pvldelta_tic_ddm_group-batch_001.csv"     # ppc_summary_{task}_{model}_{group}.csv
...
This document presents posterior predictive check (PPC) analysis for the **pvldelta_tic_ddm** model (**`r model_type`** type) applied to the **igt_mod** task with group type **batch_001**.
...
if (any(ppc_summary$category == "rt")) {
  ...
  rt_combined_plot <- plot_rt_combined(ppc_summary %>% filter(session == "session"))
...
This analysis examines how well the pvldelta_tic_ddm model can reproduce key behavioral patterns observed in the igt_mod task. The model's fit quality is evaluated through posterior predictive checks, comparing observed statistics to those generated from model simulations.
